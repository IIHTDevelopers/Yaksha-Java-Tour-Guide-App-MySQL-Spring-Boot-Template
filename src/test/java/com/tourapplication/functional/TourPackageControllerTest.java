...
import static com.tourapplication.utils.MasterData.getTourPackageDTOList;
...
public class TourPackageControllerTest {
	...
	public void testGetAllPackages() throws Exception {
		List<TourPackageDTO> tourPackageDTOS = getTourPackageDTOList();

		when(this.packageService.getAllPackages()).thenReturn(tourPackageDTOS);
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/api/packages")
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON);

		MvcResult result = mockMvc.perform(requestBuilder).andReturn();
		yakshaAssert(currentTest(),
				(result.getResponse().getContentAsString().contentEquals(MasterData.asJsonString(tourPackageDTOS))
						? "true"
						: "false"),
				businessTestFile);
	}
